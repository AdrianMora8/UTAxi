...
- Name: manage-requests.spec.ts >> Manage Requests - E2E >> authenticated >> report modal validates minimum description length
- Location: tests\e2e\manage-requests.spec.ts:207:5
...
> 7   |   await page.click('button:has-text("Iniciar Sesión")');
  ...
  105 |     });
  106 | 
  107 |     test('shows Viaje Completado when trip is COMPLETED', async ({ page }) => {
```
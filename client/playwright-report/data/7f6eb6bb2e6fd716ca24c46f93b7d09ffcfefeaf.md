...
- Name: admin-reports.spec.ts >> Admin — Gestión de Reportes - E2E >> panel admin autenticado >> ReviewModal muestra las 3 acciones disponibles
- Location: tests\e2e\admin-reports.spec.ts:107:5
...
Call log:
  - Expect "toHaveURL" with timeout 50000ms
    ...
    14 × unexpected value "http://localhost:4278/trips"
...
> 8   |   await expect(page).toHaveURL(/.*\/admin/, { timeout: 50000 });
      |                      ^ Error: expect(page).toHaveURL(expected) failed
  9   | }
  10  | 
  ...
  16  |   await expect(page).toHaveURL(/.*trips/, { timeout: 50000 });
  17  | }
  18  | 
  ...
  77  |       await expect(content.first()).toBeVisible({ timeout: 20000 });
  78  |     });
  79  | 
